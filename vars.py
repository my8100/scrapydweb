...
try:
    custom_settings_module = importlib.import_module(
        os.path.splitext(SCRAPYDWEB_SETTINGS_PY)[0]
    )
except ImportError:
    custom_data_path = ""
    custom_database_url = ""
    ...
    custom_database_url = (
        custom_database_url if isinstance(custom_database_url, str) else ""
    )
    custom_max_running_hours = getattr(custom_settings_module, "MAX_HOURS", "")
# For data storage
...
# For database
DATABASE_URL = (
    custom_database_url or default_database_url or "sqlite:///" + DATABASE_PATH
)
...
UA_DICT = {
    ...
    "iPad": (
        "Mozilla/5.0 (iPad; CPU OS 12_1_4 like Mac OS X) "
...
# Set custom hours in the scrapydweb_settings file
MAX_HOURS = custom_max_running_hours or 3
