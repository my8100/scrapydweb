...
class ScheduleView(BaseView):
        ...
        if self.task_id:
            ...
            if not self.task:
                ...
                return render_template(
                    self.template_fail, node=self.node, message=message
                )
            self.query_task()
        elif self.POST:
    ...
    def query_task(self):
        ...
        self.kwargs["expand_settings_arguments"] = (
            len(settings_arguments) > 1 or settings_arguments["setting"]
        )
        ...
        self.kwargs["USER_AGENT"] = dict((v, k) for k, v in UA_DICT.items()).get(
            USER_AGENT, ""
        )
        for k in ["ROBOTSTXT_OBEY", "COOKIES_ENABLED"]:
            v = settings_dict.pop(k, "")
            self.kwargs[k] = v if v in ["True", "False"] else ""
        self.kwargs["CONCURRENT_REQUESTS"] = settings_dict.pop(
            "CONCURRENT_REQUESTS", ""
        )
        ...
        self.kwargs["day_of_week"] = [
            str(s.strip()) for s in task.day_of_week.split(",")
        ] or [
            "*"
        ]  # 'mon-fri,sun'
        ...
        self.kwargs["coalesce"] = (
            task.coalesce if task.coalesce in ["True", "False"] else "True"
        )
    ...
    def update_kwargs(self):
        self.kwargs.update(
            dict(
                ...
                graphHTML=self.scheduled_spiders_graph(),
                ...
                url_listversions=url_for(
                    "api",
                    node=self.node,
                    opt="listversions",
                    project="PROJECT_PLACEHOLDER",
                ),
                url_listspiders=url_for(
                    "api",
                    node=self.node,
                    opt="listspiders",
                    project="PROJECT_PLACEHOLDER",
        ...
        self.kwargs.setdefault(
            "expand_settings_arguments", self.SCHEDULE_EXPAND_SETTINGS_ARGUMENTS
        )
        ...
        self.kwargs.setdefault(
            "USER_AGENT",
            "" if self.SCHEDULE_USER_AGENT is None else self.SCHEDULE_USER_AGENT,
        )
        self.kwargs.setdefault(
            "ROBOTSTXT_OBEY",
            ""
            if self.SCHEDULE_ROBOTSTXT_OBEY is None
            else self.SCHEDULE_ROBOTSTXT_OBEY,
        )
        self.kwargs.setdefault(
            "COOKIES_ENABLED",
            ""
            if self.SCHEDULE_COOKIES_ENABLED is None
            else self.SCHEDULE_COOKIES_ENABLED,
        )
        self.kwargs.setdefault(
            "CONCURRENT_REQUESTS",
            ""
            if self.SCHEDULE_CONCURRENT_REQUESTS is None
            else self.SCHEDULE_CONCURRENT_REQUESTS,
        )
        self.kwargs.setdefault(
            "DOWNLOAD_DELAY",
            ""
            if self.SCHEDULE_DOWNLOAD_DELAY is None
            else self.SCHEDULE_DOWNLOAD_DELAY,
        )
        # additional = "-d setting=CLOSESPIDER_TIMEOUT=60\r\n-d setting=CLOSESPIDER_PAGECOUNT=10\r\n-d arg1=val1"
        self.kwargs.setdefault("additional", self.SCHEDULE_ADDITIONAL)

        self.kwargs.setdefault(
            "expand_timer_task", "add_task" in request.args
        )  # '+' button in the TimeTasks page
    ...
    def scheduled_spiders_graph(self):
        # self.logger("Getting data from the SQLite jobs.db")

        con, db_type = mtd.db_connect(
            DATABASE_URL, return_db_type=True, database="timer_tasks"
        )
        query = "SELECT spider, selected_nodes as node, hour FROM task"
        if db_type == "sqlite":
        ...
        # self.logger("Transforming the data...")

        df["hour"] = df["hour"].apply(lambda x: x.zfill(2))

        hours = [str(hour).zfill(2) for hour in np.arange(24)]
        nodes = list(df.node.unique())

        nodes_scheduled_spiders = {
            node: {
                "spiders": [list(df.spider[df.hour == hour]) for hour in hours],
                "count": [df.hour[df.hour == hour].count() for hour in hours],
            }
            for node, df in {node: df[df["node"] == node] for node in nodes}.items()
        }

        # self.logger('\t...Done !')
        # self.logger('Ploting the data...')

        fig = go.Figure()

        for node, data in nodes_scheduled_spiders.items():
            spiders = ["<br>".join(spider) for spider in data["spiders"]]
            count = data["count"]
            ...
            fig.add_trace(
                go.Bar(
                    ...
                    name=node,
                    customdata=spiders,
                    hovertemplate="<b>Hour: %{x}h<br>Count: %{y}<br>Spiders:</b><br>%{customdata}",
                )
            )

        fig.update_xaxes(title_text="Time")
        fig.update_yaxes(title_text="Nb spiders")
        fig.update_layout(
            title="Number of spider per hour for each node", barmode="overlay"
        )
...
class ScheduleCheckView(BaseView):
    ...
    def dispatch_request(self, **kwargs):
        ...
        return self.json_dumps(
            {"filename": self.filename, "cmd": cmd}, as_response=True
        )

    def prepare_data(self):
        for k, d in [
            ("project", "projectname"),
            ("_version", self.DEFAULT_LATEST_VERSION),
            ("spider", "spidername"),
        ]:
        ...
        if additional:
            parts = [
                i.strip()
                for i in re.split(r"-d\s+", re.sub(r"[\r\n]", " ", additional))
                if i.strip()
            ]
            for part in parts:
                ...
                if "=" not in part:
                    continue
                m_setting = re.match(
                    r"setting=([A-Z_]{6,31}=.+)", part
                )  # 'EDITOR' 'DOWNLOADER_CLIENTCONTEXTFACTORY'
                if m_setting:
        ...
        _filename = "{project}_{version}_{spider}".format(
            project=self.data["project"], version=_version, spider=self.data["spider"]
        )
    ...
    def get_int_from_form(self, key, default, minimum):
        ...
        try:
            ...
            self.logger.warning(
                "%s. The value of request.form['%s'] would be set as %s",
                err,
                key,
                default,
            )
    ...
    def update_data_for_timer_task(self):
        ...
        self.data["__task_data"] = dict(
            ...
            or None,  # (str) – the description of this job   None
            ...
            or "*",  # From browser: "*"|"*,mon-fri"|"", May be '0',
            ...
            or None,  # (datetime.tzinfo|str)     defaults to scheduler timezone
            ...
            misfire_grace_time=self.get_int_from_form(
                "misfire_grace_time", 600, minimum=0
            )
            or None,  # (int)
            coalesce=(request.form.get("coalesce") or "True") == "True",  # (bool)
            # TypeError: max_instances must be a positive integer
            max_instances=self.get_int_from_form(
                "max_instances", 1, minimum=1
            ),  # (int)
        )
...
class ScheduleRunView(BaseView):
    ...
    def handle_form(self):
        self.selected_nodes_amount = request.form.get(
            "checked_amount", default=0, type=int
        )
        ...
        if self.selected_nodes_amount:
            ...
            self.first_selected_node = self.selected_nodes[0]
            self.url = (
                "http://%s/schedule.json"
                % self.SCRAPYD_SERVERS[self.first_selected_node - 1]
            )
    ...
    def handle_action(self):
        ...
        if self.task_data:  # For timer task
            ...
            self.task_id = self.task_data.pop(
                "task_id"
            )  # 0|positive int from edit button in the Timer Tasks page
            self.to_update_task = (
                self.task_data.pop("replace_existing") and self.task_id
            )  # replace_existing: bool
            self.db_insert_update_task()
            self.add_update_task()
        else:
            self._action = "run"
            status_code, self.js = self.make_request(
                self.url, data=self.data, auth=self.AUTH
            )
    ...
    def db_process_task(self):
        ...
        self.task.settings_arguments = self.json_dumps(
            data, sort_keys=True, indent=None
        )
        ...
        self.task.coalesce = (
            "True" if self.task_data["coalesce"] else "False"
        )  # bool True would be stored as 1
    ...
    def add_update_task(self):
        ...
        self.task_data["id"] = str(
            self.task_id
        )  # TypeError: id must be a nonempty string
        # apscheduler.executors.default: Job "execute_task (trigger: cron[year='*'..." executed successfully
        self.task_data["name"] = (
            self.task_data["name"] or "task_%s" % self.task_id
        )  # To replace execute_task with name
        ...
        if self._action == "add_fire":
            ...
            if self.to_update_task:
                self.logger.info(
                    "Task #%s would be fired right after the apscheduler_job is updated",
                    self.task_id,
                )
            else:
        ...
        try:
            ...
            if self.to_update_task:
                ...
                if self._action == "add_fire":
                    self.logger.info(
                        "Modifying next_run_time of updated task #%s to fire it right now",
                        self.task_id,
                    )
            ...
            apscheduler_logger.warning(
                "%s job_instance: \n%s",
                "Updated" if self.to_update_task else "Added",
                self.json_dumps(job_instance_dict),
            )
        finally:
            if (
                "next_run_time" in self.task_data
            ):  # TypeError: Object of type datetime is not JSON serializable
                self.task_data["next_run_time"] = str(
                    self.task_data["next_run_time"] or self.NA
                )
            self.add_task_message = (
                ...
                msg=msg,
                kwargs=self.json_dumps(kwargs),
                task_data=self.json_dumps(self.task_data),
            )
    ...
    def generate_response(self):
        if self._action in ["add", "add_fire", "add_pause"]:
            if self.add_task_result:
                return redirect(
                    url_for("tasks", node=self.node, flash=self.add_task_flash)
                )
            else:
                return render_template(
                    self.template_fail,
                    ...
                    tip=(
                        "Check out the HELP section in the Run Spider page, and then "
                        "go back to the Timer Tasks page to re-edit task #%s. "
                    )
                    % self.task_id,
        ...
        if self.js["status"] == self.OK:
            ...
            if self.selected_nodes_amount > 1:
                alert = (
                    "Multinode schedule terminated, "
                    "since the first selected node returned status: "
                    + self.js["status"]
                )
            else:
