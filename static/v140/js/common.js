...
function checkLatestVersion(pageview, SCRAPYDWEB_VERSION, GITHUB_URL) {
  ...
  if (open_github && typeof latest_version !== "undefined") {
    ...
    if (latest_version != SCRAPYDWEB_VERSION) {
      ...
      if (
        window.confirm("Would you like to STAR and help improve ScrapydWeb?")
      ) {
...
function checkBrowser() {
  ...
  var isSafari =
    winNav.userAgent.indexOf("Safari") > -1 &&
    ...
    if (this.readyState == 4) {
      ...
      if (this.status == 200) {
        ...
        alert(
          "REQUEST got code: " +
            this.status +
            ", check out the log of ScrapydWeb"
        );
        if (id !== undefined) {
          my$("#" + id).innerHTML = getRequestFailHtml(
            url,
            "code",
            this.status
          );
...
function getRequestFailHtml(url, code_status, result) {
  return (
    '<a target="_blank" class="request" href="' +
    url +
    '">' +
    'REQUEST</a><em class="fail"> got ' +
    code_status +
    ": " +
    result +
    "</em>"
...
function syncFromServersPage(SCRAPYD_SERVERS_AMOUNT) {
  if (window.localStorage) {
    if (JSON.parse(localStorage.getItem("nodesSelected") || "[]").length != 0) {
      ...
      var nodesSelected = JSON.parse(
        localStorage.getItem("nodesSelected") || "[]"
      );
