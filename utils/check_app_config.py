...
HASH = "#" * 100
# r'^(?:(?:(.*?)\:)(?:(.*?)@))?(.*?)(?:\:(.*?))?(?:#(.*?))?$'
SCRAPYD_SERVER_PATTERN = re.compile(
    r"""
                                        ^
                                        (?:
                                            (?:(.*?):)      # username:
...
def check_app_config(config):
    def check_assert(
        ...
        if is_instance is int:
            ...
            should_be = "an instance of %s%s" % (
                is_instance,
                " and not empty" if non_empty else "",
            )
        ...
        kws = dict(
            key=key,
            should_be=should_be,
            containing_type=", containing elements of type %s" % containing_type
            if containing_type
            else "",
            value="'%s'" % value if isinstance(value, str) else value,
        )
        to_assert = u"{key} should be {should_be}{containing_type}. Current value: {value}".format(
            **kws
        )

        assert (
            isinstance(value, is_instance)
            and (
                not isinstance(value, bool) if is_instance is int else True
            )  # isinstance(True, int) => True
            and (value > (-1 if allow_zero else 0) if is_instance is int else True)
            and (value if non_empty else True)
            and (
                all([isinstance(i, containing_type) for i in value])
                if containing_type
                else True
            )
        ), to_assert
    ...
    try:
        ...
        assert False, (
            "SCRAPYDWEB_PORT should be a positive integer. Current value: %s"
            % SCRAPYDWEB_PORT
        )
    ...
    if config.get("ENABLE_AUTH", False):
        ...
        logger.info(
            "Basic auth enabled with USERNAME/PASSWORD: '%s'/'%s'",
            config["USERNAME"],
            config["PASSWORD"],
        )
    ...
    if config.get("ENABLE_HTTPS", False):
        ...
        logger.info(
            "Running in HTTPS mode: %s, %s",
            config["CERTIFICATE_FILEPATH"],
    ...
    config["URL_SCRAPYDWEB"] = "%s://%s:%s" % (
        _protocol,
        _bind,
        config.get("SCRAPYDWEB_PORT", 5000),
    )
    ...
    if SCRAPY_PROJECTS_DIR:
        assert os.path.isdir(SCRAPY_PROJECTS_DIR), (
            "SCRAPY_PROJECTS_DIR not found: %s" % SCRAPY_PROJECTS_DIR
        )
        logger.info(
            "Setting up SCRAPY_PROJECTS_DIR: %s", handle_slash(SCRAPY_PROJECTS_DIR)
        )
    ...
    for node, scrapyd_server in enumerate(config["SCRAPYD_SERVERS"], 1):
        ...
        if node not in jobs_table_map:
            jobs_table_map[node] = create_jobs_table(
                re.sub(STRICT_NAME_PATTERN, "_", scrapyd_server)
            )
    ...
    if LOCAL_SCRAPYD_LOGS_DIR:
        assert os.path.isdir(LOCAL_SCRAPYD_LOGS_DIR), (
            "LOCAL_SCRAPYD_LOGS_DIR not found: %s" % LOCAL_SCRAPYD_LOGS_DIR
        )
        logger.info(
            "Setting up LOCAL_SCRAPYD_LOGS_DIR: %s",
            handle_slash(LOCAL_SCRAPYD_LOGS_DIR),
        )
    ...
    SCRAPYD_LOG_EXTENSIONS = config.get(
        "SCRAPYD_LOG_EXTENSIONS", ALLOWED_SCRAPYD_LOG_EXTENSIONS
    )
    ...
    logger.info(
        "Locating scrapy logfiles with SCRAPYD_LOG_EXTENSIONS: %s",
        SCRAPYD_LOG_EXTENSIONS,
    )
    ...
    check_assert("LONG_RUNNING_SCRAPER_STOP_INTERVAL", 300, int)
    config["SCHEDULE_CUSTOM_USER_AGENT"] = (
        config["SCHEDULE_CUSTOM_USER_AGENT"] or "Mozilla/5.0"
    )
    ...
    if config.get("SCHEDULE_USER_AGENT", None) is not None:
        ...
        assert user_agent in UA_DICT.keys(), (
            "SCHEDULE_USER_AGENT should be any value of %s. Current value: %s"
            % (UA_DICT.keys(), user_agent)
        )
    ...
    if config.get("SCHEDULE_DOWNLOAD_DELAY", None) is not None:
        ...
        if isinstance(download_delay, float):
            assert download_delay >= 0.0, (
                "SCHEDULE_DOWNLOAD_DELAY should a non-negative number. Current value: %s"
                % download_delay
            )
        else:
    ...
    if config.get("EMAIL_PASSWORD", ""):
        ...
        check_assert(
            "EMAIL_USERNAME", "", str
        )  # '' would default to config['EMAIL_SENDER']
        ...
        assert re.search(EMAIL_PATTERN, EMAIL_SENDER), (
            "EMAIL_SENDER should contain '@', like 'username@gmail.com'. Current value: %s"
            % EMAIL_SENDER
        )
    ...
    if config.get("ENABLE_MONITOR", False):
        ...
        try:
            ...
            pass
        check_assert(
            "ALERT_WORKING_DAYS", [], list, non_empty=True, containing_type=int
        )
        ALERT_WORKING_DAYS = config["ALERT_WORKING_DAYS"]
        assert all(
            [not isinstance(i, bool) and i in range(1, 8) for i in ALERT_WORKING_DAYS]
        ), (
            "Element in ALERT_WORKING_DAYS should be between 1 and 7. Current value: %s"
            % ALERT_WORKING_DAYS
        )
        ...
        try:
            ...
            pass
        check_assert(
            "ALERT_WORKING_HOURS", [], list, non_empty=True, containing_type=int
        )
        ALERT_WORKING_HOURS = config["ALERT_WORKING_HOURS"]
        assert all(
            [not isinstance(i, bool) and i in range(24) for i in ALERT_WORKING_HOURS]
        ), (
            "Element in ALERT_WORKING_HOURS should be between 0 and 23. Current value: %s"
            % ALERT_WORKING_HOURS
        )
    ...
    if database_url:
        assert any(test_database_url_pattern(database_url)), (
            "Invalid format of DATABASE_URL: %s" % database_url
        )
    ...
    if JOBS_SNAPSHOT_INTERVAL:
        ...
        kwargs = dict(
            ...
            url_jobs=config["URL_SCRAPYDWEB"]
            + handle_metadata().get("url_jobs", "/1/jobs/"),
...
def create_jobs_snapshot(url_jobs, auth, nodes):
    for node in nodes:
        url_jobs = re.sub(
            REPLACE_URL_NODE_PATTERN, r"\g<1>%s/" % node, url_jobs, count=1
        )
        try:
...
def check_scrapyd_servers(config):
    ...
    SCRAPYD_SERVERS_PUBLIC_URLS = config.get("SCRAPYD_SERVERS_PUBLIC_URLS", None) or [
        ""
    ] * len(SCRAPYD_SERVERS)
    ...
    servers = []
    for idx, (server, public_url) in enumerate(
        zip(SCRAPYD_SERVERS, SCRAPYD_SERVERS_PUBLIC_URLS)
    ):
        if isinstance(server, tuple):
            assert len(server) == 5, (
                "Scrapyd server should be a tuple of 5 elements, "
                "current value: %s" % str(server)
            )
            usr, psw, ip, port, group = server
        else:
            usr, psw, ip, port, group = re.search(
                SCRAPYD_SERVER_PATTERN, server.strip()
            ).groups()
    ...
    config["SCRAPYD_SERVERS"] = [
        "%s:%s" % (ip, port) for (group, ip, port, auth, public_url) in servers
    ]
    config["SCRAPYD_SERVERS_GROUPS"] = [
        group for (group, ip, port, auth, public_url) in servers
    ]
    config["SCRAPYD_SERVERS_AUTHS"] = [
        auth for (group, ip, port, auth, public_url) in servers
    ]
    config["SCRAPYD_SERVERS_PUBLIC_URLS"] = [
        public_url for (group, ip, port, auth, public_url) in servers
    ]
...
def check_scrapyd_connectivity(servers):
    ...
    print(HASH)
    for idx, ((group, ip, port, auth, public_url), result) in enumerate(
        zip(servers, results), 1
    ):
        print(
...
def check_slack_telegram(config, service):
    ...
    alert = (
        "Fail to send text via %s, you may need to set 'ENABLE_%s_ALERT = False'"
        % (service.capitalize(), service.upper())
    )
    if service == "slack":
        url = "https://slack.com/api/chat.postMessage"
        data = dict(
            token=config["SLACK_TOKEN"], channel=config["SLACK_CHANNEL"], text=text
        )
    else:
...
def check_email(config):
    ...
    logger.debug(
        "Trying to send email (smtp_connection_timeout=%s)...",
        config.get("SMTP_CONNECTION_TIMEOUT", 30),
    )
    result, reason = send_email(**kwargs)
    if not result and os.environ.get("TEST_ON_CIRCLECI", "False") == "False":
        logger.debug(
            "kwargs for send_email():\n%s", json_dumps(kwargs, sort_keys=False)
        )
    assert (
        result
    ), "Fail to send email. Modify the email settings above or set 'ENABLE_EMAIL_ALERT = False'"
